const openfileFunc = function (editor, url) {
  let pluginName = '打开';

  let openData = function (ext) {
    //定义一个配置项
    let opt = {};
    opt.action = "word/openFile"; //必填，上传接口地址
    opt.filter = ext; //可选，文件过滤
    opt.maxsize = 500; //可选，文件大小限制，单位M
    opt.params = { ext };
    //开始上传的钩子函数
    opt.startCallback = (file)=>{
        console.log("文件开始上传", file); //file是已经开始上传的本地文件对象
    }
    //上传进度的钩子函数
    opt.progressCallback = (object)=>{
        if(object.computable){
            console.log(
                "上传中...", 
                "当前速度：" + object.speed, 
                "已上传 " + Math.round(object.progress*100)+"%"
            )
        }
    }

    //上传成功的钩子函数
    opt.successCallback = (res)=>{
      console.log("上传成功", res); //data是接口返回的数据
      if (res.code == 0) { 
        editor.setContent(res.data.value)
      } else {
        editor.notificationManager.open({
          text: res.message,
          type: 'warning',
        });
      }
    }
    opt.errorCallback = (msg) => {
      editor.notificationManager.open({
        text: msg,
        type: 'warning',
      });
    }
    //设置上传配置项
    easyuploader.setOption(opt);

    //上传
    //* 此方法，一定要在用户行为里去触发（比如用户的click事件）
    //* 如果上传配置项实现了钩子函数，这里就不需要处理then和catch了
    easyuploader.upload();
  };
  let loadPdf = () => {
    let input = document.createElement("input");
    input.type = "file";
    input.click();
    input.onchange =  async () => { 
      console.log(input.files[0])
      let file = input.files[0];
      let blob = new Blob([file]);
      let url = window.URL.createObjectURL(blob);
      let pdf = await pdfjsLib.getDocument(url).promise;
      let numPages = pdf.numPages;
      console.log(pdf)
      let arr = [];
      for (let i = 0; i < numPages; i++) { 
        let page = await pdf.getPage(i + 1);
        //console.log(page)
        let viewport = page.getViewport({ scale: i + 1 })
        console.log(viewport);

        let operatorList = await page.getOperatorList()
        let svgGraphics = new pdfjsLib.SVGGraphics(page.commonObjs, page.objs)
        svgGraphics.embedFonts = true
        let svg = await svgGraphics.getSVG(operatorList, viewport)
        arr.push(svg.toString())
      }
      editor.setContent(arr.join(''))
      console.log(arr)
      // Array(numPages)
      //   .fill()
      //   .map(async (_, i) => {
          
      //     await editor.insertContent(svg)
      //     //div.appendChild(svg);
      //   })
      //await editor.insertContent(div)
      //console.log(div)
      // pdfjsLib.getDocument(url).promise.then(pdf => { 
      //   let numPages = pdf.numPages;
      //   console.log(numPages);
      //   Array(numPages)
      //     .fill()
      //     .map((_, i) => { 
      //       pdf.getPage(i + 1).then(page => { 
      //         console.log(res)
      //       })
      //     })
        /*
        let pages = Array(numPages)
          .fill()
          .map((_, i) => pdf.getPage(i + 1)),
          allObjects = pages.map(() => []),
          pagesScale = Array(numPages).fill(1);
        console.log(pagesScale)
        console.log(allObjects)
        console.log(pages)*/
      //})
      //let pdf = pdfjsLib.getDocument(url).promise;
      
      //const docx2html = require("docx2html")
      //docx2html(input.files[0]).then(html => { 
        //console.log(html.asZip())
        //editor.setContent(html.toString())
      //})
    }
  }
  let svgStr = `<svg t="1640280779711" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3202" width="20" height="20"><path d="M275.84 160H124.192l1.824 580.096a32 32 0 0 1-64 0L60.16 160a64 64 0 0 1 64-64h177.92l97.28 96h498.272a32 32 0 1 1 0 64H373.152l-97.28-96z" p-id="3203"></path><path d="M127.52 735.616a32 32 0 0 1-63.04-10.944l54.624-314.144C127.904 359.936 174.784 320 225.76 320h668.864c55.36 0 93.92 46.72 84.384 101.472l-72.32 416C897.824 888.064 850.976 928 800 928H160a32 32 0 0 1 0-64h640c19.712 0 40.096-17.376 43.616-37.472l72.32-416c2.784-15.904-5.984-26.528-21.312-26.528H225.76c-19.68 0-40.096 17.376-43.584 37.472l-54.656 314.144z" p-id="3204"></path></svg>`;

  editor.ui.registry.getAll().icons.openFile || editor.ui.registry.addIcon('openFile', svgStr);


  let easyOpenFile = (ext) => {

  }
  editor.ui.registry.addNestedMenuItem('openFile', {
    icon: 'openFile',
    text: '打开',
    getSubmenuItems: function () {
      return [
        {
          text: 'Word文件',
          type: 'menuitem',
          onAction: function () {
            console.log('Word文件');
            openData('doc,docx')
            //openLocalFile()
          }
        },
        {
          text: 'PDF文件',
          type: 'menuitem',
          onAction: function () {
            console.log('PDF文件');
            loadPdf()
          }
        },
        {
          text: 'MarkDown文件',
          type: 'menuitem',
          onAction: function () {
            console.log('md文件');
            openData('md')
          }
        },
        {
          text: 'text文件',
          type: 'menuitem',
          onAction: function () {
            console.log('text文件');
            openData('md')
          }
        },
        {
          text: 'RTF文件',
          type: 'menuitem',
          onAction: function () {
            console.log('RTF文件');
            openData('rtf')
          }
        },
        {
          text: 'html文件',
          type: 'menuitem',
          onAction: function () {
            console.log('html文件');
            openData('html')
          }
        },
      ]
    }
  });
  return {
    getMetadata: function () {
      return {
        name: pluginName,
        url: "https://godo.im",
      };
    }
  };
}
tinymce.PluginManager.add('openFile', openfileFunc);

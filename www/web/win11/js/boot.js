...
function skipBoot() {
    ...
    document.body.style.background = "#fff url(/web/wallpaper/img18.webp) center center/cover no-repeat";
}
// skipBoot();
...
var timer = setInterval(() => {
    ...
    //console.log(hours)
    let timeString = padStr(hours) + ":" + padStr(minute);
    ...
    let dateString = dayName + "  " + year + "/" + padStr(month) + "/" + padStr(today);
    ...
    function askPassword() {
        ...
        passwordField.addEventListener("keyup", event => {
            if (passwordField.value == "12345") {
                ...
                lockScreen.remove();
                if (firstBoot) {
                    setTimeout(notify, 1e3, "/web/icons/settings.svg", "Settings", "Welcome to Windows 11!", "Congratulations You have successfully Booted Windows 11")
                    setTimeout(_ => {
                        // notifies firefox uses to enable backdrop-filter flag
                        if (window.navigator.userAgent.indexOf("Firefox") > -1) {
                            if (CSS.supports("(backdrop-filter:blur(20px))") != true) {
                                notify("/web/icons/settings.svg", "Settings", "Hello Firefox User", "To get full UI experience please go to \"<span style=user-select:all>about:config</span>\" and search for \"layout.css.backdrop-filter.enabled\" and set it to \"true\"")
                            }
                        }
                    }, 2e3)
                    setTimeout(notify, 10e3, "/web/icons/edge.svg", "Microsoft Edge", "All new Browser is here", "Your Edge browser got updated for Windows 11 \n Please try it out.")
                    startTutorials();
                    setCookie("firstBoot", "0");
                }
                if (getCookie("cookiesAccepted") == "") {
                    notify("/web/icons/settings.svg", "Settings", "Windows 11 uses Cookies", "To improve user experience this Windows 11 uses cookies", "Accept", `setCookie('cookiesAccepted','1');notificationClose('#n${nId}')`);
                }

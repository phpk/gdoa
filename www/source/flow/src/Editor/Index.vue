  ...
  export default {
    ...
    methods: {
      async init () {
        ...
        document.querySelector('.g6-grid-container').style.inset = '0'
        ...
        // 绑定数据
          if(flowId) {
            let res = await axios.get(apiUrl + 'flow/editBefore?id=' + flowId, {headers : {rttoken}})
            //console.log(res)
            if (res && res.status == 200) {
                  let store = res.data.data.content
                  //console.log(store)
                  //_t.editor.data(JSON.parse(store))
                  console.log(JSON.parse(store))
                  titleVal = res.data.data.title
                  //_t.doRevert(JSON.parse(store))
                  _t._updateData(store)
      ...
      _updateData(store) {
        let _t = this;
        const fileJson = JSON.parse(store)
        // 清空画布
        _t.editor.clear()
        // 更新currentItem
        _t.currentItem = []
          ...
          case 'savedata' : {
            ...
            _t.$Modal.confirm({
                ...
                onOk: async () => {
                  ...
                  //console.log(content)
                  if(flowId) {
                    await axios.post(apiUrl + 'flow/edit', {
                        id : flowId,
                        content: content,
                        title: titleVal
                    }, { headers: { rttoken }, withCredentials: true })
                  }else{
                    let res = await axios.post(apiUrl + 'flow/add', {
                        content: content,
                        title: titleVal
                    }, { headers: { rttoken }, withCredentials: true })
                    flowId = res.data.data;
                  }

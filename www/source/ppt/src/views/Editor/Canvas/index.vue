...
export default defineComponent({
  ...
  setup() {
    ...
    const useServerStore = useServer()
    const pptId = useServerStore.getQuery('id')
    if(pptId) {
      useServerStore.getJSON(pptId).then(res => {
        //console.log(res)
        if(res && res.status == 200) {
          const resData = res.data.data;
          slidesStore.setSlides(JSON.parse(resData.content))
          useServerStore.setTitle(resData.title)
        }
      })
    }else{
      useServerStore.clearTitle()
    }

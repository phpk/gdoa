...
const setToken = (res) => {
    ...
    localStorage.setItem('_godocmsUserRoute', JSON.stringify(res.routeData))
}
const getRoute = () => {
    let data = localStorage.getItem('_godocmsUserRoute');
    if (data) {
...
const loginOutToken = () => {
    localStorage.removeItem("_godocmsUserRoute");
...
const errorStatus = (xhr, layui) => {
    ...
    if (xhr.status === 400) {
        ...
        top.location.href = '/member/index.html';
    }

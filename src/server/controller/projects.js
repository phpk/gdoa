...
module.exports = class extends Base {
    ...
    async editAction() {
        let post = this.post();
		let condition = { project_id : post.project_id };
    ...
    async editBeforeAction() {
        let id = this.get('id');
        let data = await this.model('project').where({ project_id: id }).find()
    ...
    async delAction() {
        let id = this.post('id');
        if (!await this.hasData('project', { project_id: id }))
            return this.fail('数据不存在')
        await this.model('project').where({ project_id : id }).delete()
        return this.success()
    }
}
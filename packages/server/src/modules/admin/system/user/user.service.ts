...
export class SysUserService {
   ...
   * @param password 用户密码
   */
  async updateSalt(userId: number, password: string): Promise<void> {
    const salt = this.util.generateRandomValue(32);
    const newPassword = this.util.md5(`${password}${salt}`);
    await this.userRepository.update({ id: userId }, { password: newPassword, psalt: salt });
  }
  /**
